    ...
    <div class="z-search-box">
      <el-form ref="form" :model="form" label-width="120px">
        <el-row>
          ...
          <el-col :span="8">
            <div class="z-span-20">
              <el-form-item label="交易类型：" prop="balanceType">
                <el-select name="type" v-model="form.balanceType">
  ...
  export default {
    data () {
      return {
        ...
        totalCount: 0,
        currentPage: 1,
        // statusOptions:TOOLS.fetchObj("commonObj").BalanceStatus,//状态
        statusOptions: [
          {'label': '全部', 'value': ''},
          ...
          {'label': '正在结算', 'value': 30},
          {'label': '已经执行', 'value': 40},
          {'label': '失败', 'value': [50, 3000]}
        ], // 状态
        balanceTypeOptions: TOOLS.fetchObj('commonObj').PayBalanceType, // 交易类型
        form: {
          productId: '', // 项目ID
          productName: '', // 项目名称
          orderNo: '', // 订单号
    ...
    mounted () {
      ...
      if (this.getFunctionList.length > 0) {
        for (let i = 0, len = this.getFunctionList.length; i < len; i++) {
          if (this.getFunctionList[i].label === '分账查询') {
    ...
    methods: {
      ...
      // 交易类型转换
      formatterBalanceType (row, column) {
        return TOOLS.valueToLabel(this.balanceTypeOptions, row[column.property])
      },
      // 状态转换
      ...
      objFn () {
        ...
        let obj = {
          ...
          'balanceType': this.form.balanceType// 交易类型
        }
        return obj
      },
      ...
      // 请求数据
      getTableList () {
        API.$pageQueryBalance(TOOLS.noEmetyProp(this.objFn()), res => {
