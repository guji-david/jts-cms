  ...
  <div id="content-body" class="content-body">
    ...
    <el-table
      ...
      >
        <template scope="scope">
          <span v-if="scope.row.remark !== null &&  scope.row.remark !==''">
            <el-button @click="goRefundAdvance(scope.row)" type="text" :disabled="repayBtn===scope.row.paymentNo">退款</el-button>
          </span>
          <span v-else>
            ——
          </span>
    ...
    mounted () {
      if (this.getFunctionList.length > 0) {
        for (let i = 0, len = this.getFunctionList.length; i < len; i++) {
          if (this.getFunctionList[i].label === '入账查询') {
      ...
      goRefundAdvance (obj) {
        API.$refunds(obj, res => {
          if (res.success) { // 如果成功的话
            let _this = this
            setTimeout(function () {
              _this.getTableList(_this.tabName, _this.pageNo, _this.pageSize)
            }, 100)
            TOOLS.alertConfirm(this, '', `退款请求已发送`, '', '确定', '', 'success', false)
          } else {
            TOOLS.alertConfirm(this, '', `${res.resultMsg} 退款请求已发送失败`, '', '确定', '', 'warning', false)
          }
        })
      },
      // 查询
