      ...
      <!-- 搜索结束 -->
        ...
        >
            <template scope="scope">
                <span @click="toDetailPage(scope.row,'delayAlreadyRepayInfo')" class="detailSpan">{{scope.row.productName}}</span>
            </template>
        </el-table-column>
        <el-table-column
            prop="productId"
...
export default {
  ...
  methods: {
    ...
    goRepay(row){
        this.$router.push({name: 'DelayRepaymentInfo',params:{id: row.productId}})
    }
  }
};
</script>
