<template>
  <div id="content-body" class="content-body">
    ...
    <div class="z-search-box">
      <el-form :model="androidForm" label-width="120px" ref="androidForm">
        <el-row>

          <el-col :span="8">
            <el-form-item label="版本号：" prop="version">
              <el-input v-model.trim="androidForm.version" placeholder="请输入Android版本名称"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="强制升级：" prop="isUpdate">
              <el-select name="type"v-model="androidForm.isUpdate">
          ...
          <el-col :span="8">
            <div class="z-span-20" style="padding-left:20px">
              ...
              <span class="z-btn-span"><el-button @click="resetForm('androidForm')">重置</el-button> </span>
            </div>
    ...
    <div class="z-tool-bar">
      <span class="z-btn-span"><el-button type="default" icon="plus" @click="addAndroid" v-if="disabledArray[1]">新增</el-button></span>
    </div>
    <!-- 工具栏结束 -->
    <!-- 列表开始 -->
    <el-table
      ...
      <el-table-column
        ...
        <template scope="scope">
          <div id="cellandroid">
            <el-button @click="editAndroid(scope.row)"type="text" v-if="disabledArray[2]">预览</el-button>
            <el-button @click="delAndroid(scope.$index,scope.row)"type="text" v-if="disabledArray[3]">删除</el-button>
          </div>
    ...
    <el-dialog title="提示" v-model="delDialogVisible" size="tiny">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="delAndroidBtn" :disabled="delDisabled">确 定</el-button>
      </span>
    </el-dialog>
    <!-- 预览详情 -->
    <el-dialog title="预览" v-model="preDialogVisible" size="tiny">
      <div >
        <el-form  :model="androidObj" label-width="160px" ref="androidObj">
          <el-form-item label="当前版本号：">
                <span>{{androidObj.version}}</span>
          </el-form-item>
          <el-form-item label="版本转换值：">
            <span>{{androidObj.versionKey}}</span>
          </el-form-item>
          <el-form-item label="md5：">
            <span>{{androidObj.md5}}</span>
          </el-form-item>
          <el-form-item label="下载保存名称：" >
            <span>{{androidObj.downloadName}}</span>
          </el-form-item>
          <el-form-item label="APK资源包：" >
            <span>{{androidObj.downloadName}}</span>
          </el-form-item>
          <el-form-item label="文件大小(M)：">
            <span>{{androidObj.fileSize}}</span>
          </el-form-item>
          <el-form-item label="当前版本描述：" >
            <span>{{androidObj.content}}</span>
          </el-form-item>
          <el-form-item label="是否强制升级：">
            <span>{{androidObj.isUpdate}}</span>
          </el-form-item>


          <div style="text-align: center">

            <el-button type="primary":disabled="isValidDisabled" @click="saveForm(androidObj.isValid)">{{confirmText}}</el-button>
    ...
    methods: {
      ...
      getTableList (pageNo, pageSize) {
        var obj = {
          'pageNo': pageNo,
          'pageSize': pageSize,
          'version': this.androidForm.version,
          'isUpdate':this.androidForm.isUpdate
        }
        API.$androidPageQuery(obj, res => {
      ...
      addAndroid () {
        TOOLS.clearProData()
        this.$router.push({name: 'EditAndroid', params: { androidId: 0 }})
      },
      // 预览
      editAndroid (row) {
        ...
        this.androidQueryById(this.currentId);
      },
      //通过id获取当前id的信息
      androidQueryById(id){
        API.$androidQueryById(id, res => {
          if (res.success) {
            this.androidObj = res.data;
            this.androidObj.isUpdate = TOOLS.valueToLabel(this.isUpdateOptions, this.androidObj.isUpdate)
      ...
      saveForm(isValid){
        if(isValid==10){
          this.isValidDisabled=true;
          API.$androidUpdate(this.currentId, res => {
            if (res.success) {
      ...
      delAndroidBtn () {
        ...
        API.$androidDel(this.currentId, res => {
