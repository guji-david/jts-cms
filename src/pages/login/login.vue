  ...
  <div class="login-wrap">
    ...
    <div class="ms-login">
      <el-form :model="loginObj" label-width="80px"  ref="loginObj":rules="rules">
        ...
        <div class="login-btn">
        <el-button type="primary"@click="signIn('loginObj')":disabled="loginDiabled" v-loading.fullscreen.lock="fullscreenLoading">登录</el-button>
        </div>
      </el-form>
    </div>
    ...
    methods: {
      ...
      signIn (formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.loginDiabled = true
            API.$login(this.loginObj.name, this.loginObj.pwd, this.loginObj.code, res => {
              if (res.success) {
                if (res.data.permissionTrees.length) {
                  TOOLS.setList('subnavData', res.data.permissionTrees)
                  this.JSONlist = TOOLS.getNavDateToSimpleJson(TOOLS.getList('subnavData'))// 将树形导航嵌套的json转换成单层json
                  TOOLS.setList('simpleJsonSubnavData', this.JSONlist)
      ...
      commonDataInit () {
        API.$commonDataInit(res => {
          var commonObj = this.addLabelAll(res)
         /* let newArr = []
          for (let i = 0, len = commonObj.ProductType.length; i < len; i++) {
            if (commonObj.ProductType[i].value !== 'exportRebate') {
              newArr.push(commonObj.ProductType[i])
            }
          }
          commonObj.ProductType = newArr*/

          TOOLS.saveObj('commonObj', commonObj)
          TOOLS.setDomain(TOOLS.fetchObj('commonObj').domain[1].value)
          if (this.$route.query.redirect) {
            let activeRouterObj = TOOLS.getIdByRouter(this.JSONlist, this.$route.query.redirect)
            if (activeRouterObj) {
              this.$store.commit('updateActiveIndex', 0)
              this.$store.commit('addTag', { label: activeRouterObj.label, router: activeRouterObj.router, id: activeRouterObj.id })
              this.$store.commit('addBreadcrumb', activeRouterObj.id)
              this.$store.commit('addFunctionList', activeRouterObj.id)
